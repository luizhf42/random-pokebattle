var pokemon = [
    ...
    function sortearPokemon() {
        ...
        var numeroPokemonCPU = parseInt(Math.random() * pokemon.length);
        pokemonCPU = pokemon[numeroPokemonCPU];
        pokemon.splice(numeroPokemonCPU, 1);
        
        var numeroPokemonJogador = parseInt(Math.random() * (pokemon.length - 1));
        ...
        if (pokemon.length > 0) {
            ...
            document.getElementById("pokemon-jogador").innerHTML = "Todos os pokémon já foram usados, e ainda estamos implementando o replay. Caso queira jogar novamente, por favor, recarregue a página!"
        }
    }
